...
(
	...
	//test_data_type
	  // RELU signals
    output logic [31:0]          relu_addr_base,
    output logic [31:0]          relu_len,
    output logic [4:0]           relu_cnt,
...
///RELU 
// RELU state machine with enough bits
enum logic [1:0] {
    IDLE_RELU  = 2'b00,
    RELU_READ  = 2'b01,
    RELU_WRITE = 2'b10
...
/////RELU
// --- RELU small FSM: in-place ReLU on M_cnn_Cache[0..15] ---
// Add this after the other FSMs (gemm/mp) but inside the module.
...
always_ff @(posedge clk or negedge rst_n) begin
    if (~rst_n) begin
        ...
        case (relu_cs)
            ...
            RELU_READ: begin
                if (M_cnn_Cache[relu_cnt] < 0)
                    M_cnn_Cache[relu_cnt] <= 0;
                relu_cnt <= relu_cnt + 1;
            end
...
// expose active signal
// you may want to connect this to top-level debug outputs or use it in scnn_ready_o logic
// e.g. assign relu_active_o = relu_active;
