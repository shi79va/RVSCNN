...
enum logic [3:0] {
	...
	RELU_CAL  // <-- new state   RELU CUSTOM INSTRUCTION
	...
	if (~rst_n)begin
		...
		case(gemm4x4_cs)
			...
			FINISH_GEMM: begin
				...
				// if(operator_i == RELU_OP && enable_i) begin
				// 	for(i=0; i<16; i=i+1) begin
				// 		M_cnn_Cache[i] <= (M_cnn_Cache[i][31] ? 32'd0 : M_cnn_Cache[i]);
				// 	end
				// end
				// if (operator_i == RELU_OP && enable_i) begin
			...
			RELU_CAL: begin
				...
				gemm4x4_cs <= FINISH_GEMM;  // go back to FINISH_GEMM after RELU
			end
//////////////////////////////////////////////////
			default:;
		endcase
...
always_comb begin
	...
	case (gemm4x4_cs)
		...
		/////////////RELU CUSTOM INSTRUCTION
		RELU_CAL: begin
			gemm4x4_ready = 1'b0;   // still active
			gemm4x4_active = 1'b1;
			gemm4x4_get_data = 1'b0;
		end
/////////////////////////////////////////
